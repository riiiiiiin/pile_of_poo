...
public:
    ...
    inline int active_player() const override {
        return steps & 1;
    }
...
private:
    ...
    std::vector<double> simulate_from(GameState state) const {
        ...
        int action_id;
        r_selection.initialize(state.n_actions(),0);
        while(not state.done()){
            for(int i=0;i<state.n_actions();i++){
                int dx=abs(state.action_space()[i]/11-last_action_/11),dy=abs(state.action_space()[i]%11-last_action_%11);
                r_selection.submit(1.0/(dx+dy));
            }
            action_id=r_selection.selected_index();
...
public:
    ...
    ActionType select_action(int iterations, double exploration,int last_action){
...
// UCT探索项系数
const double exploration = 0.3;

// 迭代次数
const int iterations = 5000;

// 搜索生成state下的动作
Json::Value get_next_action(const HexState<N>& state, bool forced_flag,int last_action){
    ...
    if (forced_flag){
        ...
        action = mcts.select_action(iterations, exploration,last_action);
    }
...
int main(){
    ...
    result["response"] = get_next_action(state, forced_flag,x*N+y);
